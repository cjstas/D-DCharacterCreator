...
public class ListMaker {
    ...
    public ArrayList<Spell> returnSpellsArrayList5e() {
        ...
        try {
            unSortedSpells = new String(Files.readAllBytes(spells.toPath()));
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for(String instance: SpellData) {
            ...
            int level = Integer.parseInt(SpellInstanceData[1]);
            String unpolishedClasses = SpellInstanceData[2].replaceAll(",","");
            unpolishedClasses = unpolishedClasses.replaceAll("\\p{P}","");
            String[] classes = unpolishedClasses.split(" ");
            ArrayList<String> useableBy = new ArrayList<>();
            Collections.addAll(useableBy, classes);

            Spell spell = new Spell(Name, useableBy, level);
            output.add(spell);
        }
